...
static char *test_constructs() {
    struct hashmap_qp *map = hashmap_qp_new(hasher, value_free);
...
static char *test_inserts() {
    ...
    for (size_t i = 2; i < 6; ++i) {
        hashmap_qp_insert(map, i, NULL);
        mu_assert("error, entries count must be equal to i",
                  map->entries_count == i);
        mu_assert("error, slots count must be equal to 10",
                  map->slots_count == 10);
    }
    hashmap_qp_insert(map, 6, NULL);
    mu_assert("error, entries count must be equal to 6",
              map->entries_count == 6);
